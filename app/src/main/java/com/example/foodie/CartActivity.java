...
import static com.example.foodie.MyListAdapter.*;
...
import com.example.foodie.UDP.TDataSender;
...
//        Log.d("table_numberCart", "Received table: " + tableNumber);
        ...
        ArrayList<Integer> p_idList = new ArrayList<>();
...
// Convert ArrayLists to arrays
        ...
        int[] p_ids = null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            p_ids = p_idList.stream().mapToInt(i -> i).toArray();
        }
...
// Now pass to your adapter
        MyListAdapter adapter = new MyListAdapter(this, names, qtys, rates, totals,p_ids);
        ...
        placeOrder.setOnClickListener(e -> {
            ...
            TDataSender tr = new TDataSender();

            // Save to database
            for (MenuItem item : cartItems) {
                // new DB method: insertItem(String name, int quantity, double price, int tableNo)
                myDatabase.insertItem(item.getName(), item.getQuantity(), item.getPrice(), tableNumber,item.getP_id());
                tr.sendOrderData(item.getQuantity(),item.getP_id(),tableNumber);
            }
