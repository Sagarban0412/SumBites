...
//        List<MenuItem> drinkMenu = db.getItemsByCategory("special");
//
...
public class SpecialFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        MyDatabase db = new MyDatabase(requireContext());
        List<MenuItem> specialMenu = db.getItemsByCategory("special");

        RecyclerView recyclerView = view.findViewById(R.id.recycler_menu);
        recyclerView.setLayoutManager(new LinearLayoutManager(requireContext()));
        recyclerView.setAdapter(new MenuAdapter(specialMenu, requireContext(), tableNumber));
